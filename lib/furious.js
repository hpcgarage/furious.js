...
var AsmJSContext = require("./js/AsmJSContext");
var AsmJSWorker = require("./js/AsmJSWorker");
var PNaClContext = require("./PNaClContext");
var WebCLContext = require("./webcl/WebCLContext");
var WebSocketContext = require("./websocket/WebSocketContext");
 ...
 *         <tr>
 *             <td>"websocket"</td>
 *             <td>WebSocket backend. Works in all modern browsers. This backend executes computations on a cloud server through Web Socket connection. The server and connection require pre-configuration.</td>
 *         </tr>
 *     </table>
 *
...
var init = function(backend, options, callback) {
	...
	if (backend === "javascript") {
		...
		if (typeof async === "undefined") {
			async = AsmJSWorker.isSupported();
		}
		if (async) {
			options.baseUrl = currentScriptDir;
			return new AsmJSWorker(options, callback);
		} else {
			return new AsmJSContext(options, callback);
		}
		...
		options.baseUrl = currentScriptDir;
        return new WebSocketContext(options, callback);
    } else {
...
var getDefaultBackend = function() {
	if (WebSocketContext.isConfigured()) {
	    return "websocket";
	} else if (WebCLContext.isUsable()) {
		return "webcl";
...
/**
 * Queries default options for the specified backend on this platform.
 *
 * @param {String} backend - name of the backend to query options for.
 *
 * @static
...
var getDefaultBackendOptions = function(backend) {
	if (backend === "javascript") {
		...
		if (PNaClContext.isSupported()) {
			return {
				"manifest": PNaClContext.getManifestURL(currentScriptDir)
			};
		} else {
			return {};
		}
		...
		if (WebSocketContext.isSupported()) {
			return {
				"url": WebSocketContext.getDefaultURL(currentScriptDir)
			};
		} else {
			return {};
		}
 ...
 *         <tr>
 *             <td>"websockets"</td>
 *             <td>Detect if the environment supports Web Socket connections.</td>
 *         </tr>
 *         <tr>
 *             <td>"asm.js"</td>
