...
function ConnectionContext(options) {
    ...
    var eyeKernelSource = fs.readFileSync(__dirname + "/webcl/eyeKernel.cl", "utf8");
	var source = binaryKernelsSource + unaryKernelsSource +
		reductionKernelsSource + axisReductionKernelsSource +
		productKernelsSource + utilKernelsSource + eyeKernelSource;
...
ConnectionContext.prototype.createIdentityMatrix = function(requestId, idOut, rows, columns, diagonal, dataType) {
    ...
    kernel.setArg(2, new Int32Array([diagonal]));
    kernel.setArg(3, buffer);
    this.queue.enqueueNDRangeKernel(kernel, 2, null, [rows, columns, 0], null);
    var arrayOut = {
        buffer: buffer,
        shape: [rows, columns],
        length: rows*columns,
    ...
    var dataBuffer = new arrayOut.dataType.arrayType(arrayOut.length);
    this.queue.enqueueReadBuffer(arrayOut.buffer, true, 0, dataBuffer.byteLength, dataBuffer);
