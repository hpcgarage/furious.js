...
function createEmptyArray(requestId, idOut, shape, dataType) {
	...
	var arr = Module._malloc(length * size);
	Module._memset(arr, 0, length * size);	
	idMap[idOut] = {
		data: arr,
		shape: shape,
		length: length,
...
function createDataArray(requestId, idOut, shape, dataType, dataBuffer) {
	...
	var length = util.computeLength(shape);
	var size = dataType.size;
	var arr = Module._malloc(size * length);
	Module.HEAPU8.set(new Uint8Array(dataBuffer), arr);
...
function createConstArray(requestId, idOut, shape, dataType, fillValue) {
	...
	var arr = Module._malloc(length * size);
	Module._memset(arr, 0, length * size);
	Module._createConstArray(arr, dataTypeEnum[dataType.type], length, fillValue);	
...
function createIdentityMatrix(requestId, idOut, rows, columns, diagonal, dataType) {
	...
	var arr = Module._malloc(rows * columns * size);
	Module._memset(arr, 0, rows * columns * size);
	Module._fillDiag(arr, dataTypeEnum[dataType.type], rows, columns, diagonal, 1.0);
	idMap[idOut] = {
		data: arr,        
...
function linspace(requestId, idOut, start, stop, samples, closed, dataType) {
	...
	dataType = util.checkDataType(dataType);
	var size = dataType.size;
	var arr = Module._malloc(samples * size);
	Module._memset(arr, 0, samples * size);
	Module._linspace(arr, dataTypeEnum[dataType.type], start, stop, samples, (closed) ? 1 : 0);
...
function fetch(requestId, idA) {
	...
	var dataView = Module.HEAPU8.subarray(arrayA.data, arrayA.data+arrayA.dataType.size*arrayA.length);
	fetchResponse.dataBuffer = new Uint8Array(dataView).buffer;
	response.fetchResponse = fetchResponse;
	var message = response.encodeAB();
	self.postMessage(message, [message]);

	if (idA < 0) {
		Module._free(arrayA.data);
		delete idMap[-idA];
	}
}
...
binaryOperationMap[BinaryOperationRequest.Type.ADD]    = Module._add;
binaryOperationMap[BinaryOperationRequest.Type.SUB]    = Module._sub;
binaryOperationMap[BinaryOperationRequest.Type.MUL]    = Module._mul;
binaryOperationMap[BinaryOperationRequest.Type.DIV]    = Module._mdiv;
...
function binaryOperation(requestId, type, idA, idB, idOut) {
	...
	binaryOperationMap[type](arrayA.data, arrayB.data, arrayOut.data, dataTypeEnum[arrayA.dataType.type], arrayOut.length);
	if (idA < 0) {
		Module._free(arrayA.data);
		delete idMap[-idA];
	}
	if (idB < 0) {
		Module._free(arrayB.data);
		delete idMap[-idB];
	}
}

var binaryConstOperationMap = {};
binaryConstOperationMap[BinaryConstOperationRequest.Type.ADDC]    = Module._addConst;
binaryConstOperationMap[BinaryConstOperationRequest.Type.SUBC]    = Module._subConst;
binaryConstOperationMap[BinaryConstOperationRequest.Type.SUBRC]   = Module._subRevConst;
binaryConstOperationMap[BinaryConstOperationRequest.Type.MULC]    = Module._mulConst;
binaryConstOperationMap[BinaryConstOperationRequest.Type.DIVC]    = Module._divConst;
binaryConstOperationMap[BinaryConstOperationRequest.Type.DIVRC]   = Module._divRevConst;
...
function binaryConstOperation(requestId, type, idA, valueB, idOut) {
	...
	binaryConstOperationMap[type](arrayA.data, valueB, arrayOut.data, dataTypeEnum[arrayA.dataType.type], arrayOut.length);
	if (idA < 0) {
		Module._free(arrayA.data);
		delete idMap[-idA];
	}
...
var reductionMap = {};
reductionMap[ReductionRequest.Type.SUM] = Module._sum;
reductionMap[ReductionRequest.Type.MAX] = Module._max;
reductionMap[ReductionRequest.Type.MIN] = Module._min;
...
function reduction(requestId, type, idA, idOut) {
	...
	var arrayOut = idMap[idOut];
	var shapeOut = [];
	var arr = Module._malloc(arrayA.dataType.size);
	if (typeof arrayOut === "undefined") {
		arrayOut = {
			data: arr,
			shape: shapeOut,
	...
	reductionMap[type](arrayA.data, arrayOut.data, dataTypeEnum[arrayA.dataType.type], arrayA.length);
	if (idA < 0) {
		Module._free(arrayA.data);
		delete idMap[-idA];
	}
