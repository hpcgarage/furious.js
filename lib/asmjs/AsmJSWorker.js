...
var testcall = Module.cwrap('test', 'number');
var testfloat = Module.cwrap('floattest', 'number');
var idMap = {};
var idPtrMap = {};
var dataTypeMap = {};
...
function createEmptyArray(requestId, idOut, shape, dataType) {
	...
	var arrbuf, arrview, arr, retarray;
	if (dataType.type == "f32") {
		arrbuf = new ArrayBuffer(length * 4);
		arrview = new Float32Array(arrbuf);
		arr = Module._malloc(length * 4);
		//arrview[0] = 99.0;
		Module.HEAPF32.set(arrview, arr/4);
	}
	else {
		arrbuf = new ArrayBuffer(length * 8);
		arrview = new Float64Array(arrbuf);
		arr = Module._malloc(length * 8);
		//arrview[0] = 99.0;	
		Module.HEAPF64.set(arrview, arr/8);
	}
	idMap[idOut] = {
		data: arr,
		buffer: arrbuf,
	...
	/*if (dataType.type == "f32") {
		retarray = Module.HEAPF32.subarray(arr/4, arr/4+length);
	}
	else {
		retarray = Module.HEAPF64.subarray(arr/8, arr/8+length);	
	}	
...
function createDataArray(requestId, idOut, shape, dataType, dataBuffer) {
	...
	dataType = util.checkDataType(dataType);
	var length = util.computeLength(shape);
	var arrbuf, arrview, arr;
	if (dataType.type == "f32") {
		arrview = new Float32Array(dataBuffer);
		arr = Module._malloc(4 * length);
		Module.HEAPF32.set(arrview, arr/4); 
	}	
	else {
		arrview = new Float64Array(dataBuffer);
		arr = Module._malloc(8 * length);
		Module.HEAPF64.set(arrview, arr/8); 
...
function fetch(requestId, idA) {
	//var arrarE = idPtrMap[Math.abs(idA)];
	var arrayA = idMap[Math.abs(idA)];
	if (typeof arrayA === "undefined") {
		throw new Error("Invalid input ID");
	}
	if (arrayA.dataType == "f32") {
		var view = Module.HEAPF32.subarray(arrayA.data/4, arrayA.data/4+arrayA.length);
		console.log(view[0]);	
	}
	else {
		var view = Module.HEAPF64.subarray(arrayA.data/8, arrayA.data/8+arrayA.length);
		console.log(view[0]);
	}
	...
	fetchResponse.dataBuffer = arrayA.buffer;
