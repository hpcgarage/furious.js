...
enum FJS_Error FJS_Execute_CreateEmptyArray(
#ifdef __native_client__
	...
	enum FJS_DataType dataType)
{
	...
	#else
		FJS_AllocateId(idOut, array);
	#endif
	return FJS_Error_Ok;
}

enum FJS_Error FJS_Execute_CreateConstArray(
#ifdef __native_client__
	PP_Instance instance,
#endif
	uint32_t idOut,
	...
	double fillValue)
{
	...
	#else
		FJS_AllocateId(idOut, arrayOut);
	#endif
	return FJS_Error_Ok;
}

enum FJS_Error FJS_Execute_CreateDataArray(
#ifdef __native_client__
	PP_Instance instance,
#endif
	uint32_t idOut,
...
enum FJS_Error FJS_Execute_CreateIdentityMatrix(
#ifdef __native_client__
	PP_Instance instance,
#endif
	uint32_t idOut,
...
enum FJS_Error FJS_Execute_LinSpace(
#ifdef __native_client__
	PP_Instance instance,
#endif
	uint32_t idOut,
...
enum FJS_Error FJS_Execute_ReShape(
#ifdef __native_client__
	PP_Instance instance,
#endif
	int32_t idA,
	...
	struct FJS_Shape shapeOut)
{
	...
	#else
		struct NDArray* arrayA = FJS_GetPointerFromId(__builtin_abs(idA));
	#endif
	if (arrayA == NULL) {
	...
	#else
		struct NDArray* arrayOut = FJS_GetPointerFromId(idOut);
	#endif
	if (arrayOut != NULL) {
	...
	if (idA < 0) {
		...
		#else
			FJS_ReleaseId(__builtin_abs(idA));
		#endif
	}
...
enum FJS_Error FJS_Execute_Repeat(
#ifdef __native_client__
	PP_Instance instance,
#endif
	int32_t idA,
...
enum FJS_Error FJS_Execute_DeAllocate(
#ifdef __native_client__
	PP_Instance instance,
#endif
	int32_t idA)
{
	...
	#else
		struct NDArray* array = FJS_GetPointerFromId(idA);
	#endif
	if (array == NULL) {
		return FJS_Error_InvalidId;
	} else {
...
enum FJS_Error FJS_Execute_Fetch(
#ifdef __native_client__
	PP_Instance instance,
#endif
	int32_t idA,
	struct FJS_Buffer buffer[restrict static 1])
{
	#ifdef __native_client__
		struct NDArray* array = FJS_GetPointerFromId(instance, __builtin_abs(idA));
	#else
		struct NDArray* array = FJS_GetPointerFromId(__builtin_abs(idA));
	#endif
	if (array == NULL) {
		return FJS_Error_InvalidId;
	}
...
#ifdef __native_client__
enum FJS_Error FJS_Execute_Info(
	PP_Instance instance,
