...
enum FJS_Error FJS_Dispatch_Request(PP_Instance instance, const void* requestPointer, size_t requestSize) {
	...
	if (request == NULL) {
		error = FJS_Error_InvalidProtobuf;
		goto cleanup;
	}
	...
	switch (request_type) {
		case FURIOUS__REQUEST__TYPE__EMPTY_ARRAY:
		{
			...
			error = FJS_Execute_CreateEmptyArray(instance,
		...
		case FURIOUS__REQUEST__TYPE__DATA_ARRAY:
		{
			...
			error = FJS_Execute_CreateDataArray(instance,
		...
		case FURIOUS__REQUEST__TYPE__CONST_ARRAY:
		{
			...
			error = FJS_Execute_CreateConstArray(instance,
		...
		case FURIOUS__REQUEST__TYPE__IDENTITY_MATRIX:
		{
			...
			error = FJS_Execute_CreateIdentityMatrix(instance,
		...
		case FURIOUS__REQUEST__TYPE__DEALLOCATE:
		{
			...
			error = FJS_Execute_DeAllocate(instance,
		...
		case FURIOUS__REQUEST__TYPE__FETCH:
		{
			...
			error = FJS_Execute_Fetch(instance,
				fetch_request->id_a,
				&buffer);
		...
		case FURIOUS__REQUEST__TYPE__BINARY_OPERATION:
		{
			...
			error = FJS_Execute_BinaryOperation(instance,
		...
		case FURIOUS__REQUEST__TYPE__BINARY_CONST_OPERATION:
		{
			...
			error = FJS_Execute_BinaryConstOperation(instance,
		...
		case FURIOUS__REQUEST__TYPE__UNARY_OPERATION:
		{
			...
			error = FJS_Execute_UnaryOperation(instance,
		...
		case FURIOUS__REQUEST__TYPE__REDUCTION_OPERATION:
		{
			...
			error = FJS_Execute_Reduction(instance,
		...
		case FURIOUS__REQUEST__TYPE__AXIS_REDUCTION_OPERATION:
		{
			...
			error = FJS_Execute_AxisReduction(instance,
		...
		case FURIOUS__REQUEST__TYPE__DOT_OPERATION:
		{
			...
			error = FJS_Execute_DotOperation(instance,
		...
		case FURIOUS__REQUEST__TYPE__LUP_DECOMPOSITION:
		{
			...
			error = FJS_Execute_LUPDecomposition(instance,
				...
				lup_decomposition_request->id_outl,
				lup_decomposition_request->id_outu,
				lup_decomposition_request->id_outp);
			break;
		}
		case FURIOUS__REQUEST__TYPE__CHOLESKY_DECOMPOSITION:
		{
			...
			error = FJS_Execute_CholeskyDecomposition(instance,
		...
		case FURIOUS__REQUEST__TYPE__SOLVE_TRIANGULAR:
		{
			...
			error = FJS_Execute_SolveTriangular(instance,
...
cleanup:
	if (request != NULL) {
		furious__request__free_unpacked(request, NULL);
	}
	return error;
}
