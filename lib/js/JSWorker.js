...
function LUPDecomposition(requestId, idA, idOutL, idOutU, idOutP) {
	var arrayA = idMap[Math.abs(idA)];
	if (typeof arrayA === "undefined") {
		throw new Error("Invalid input ID");
	}
	...
	// Verify OutL
	var arrayOutL = idMap[idOutL];
	if (typeof arrayOutL !== "undefined") {
		if (!util.arrayEquals(arrayA.shape, arrayOutL.shape)) {
			throw new Error("Incompatible shapes");
		}
		if (!arrayA.dataType.equals(arrayOutL.dataType)) {
			throw new Error("Incompatible data type");
		}
	} else {
		arrayOutL = {
		...
		idMap[idsOutL] = arrayOutL;
	}

	// Verify OutU
	var arrayOutU = idMap[idOutU];
	if (typeof arrayOutU !== "undefined") {
		if (!util.arrayEquals(arrayA.shape, arrayOutU.shape)) {
			throw new Error("Incompatible shapes");
		}
		if (!arrayA.dataType.equals(arrayOutU.dataType)) {
			throw new Error("Incompatible data type");
		}
	} else {
		arrayOutU = {
		...
		idMap[idsOutU] = arrayOutU;
	}

	// Verify OutP
	var arrayOutP = idMap[idOutP];
	if (typeof arrayOutP !== "undefined") {
		if (!util.arrayEquals(arrayA.shape, arrayOutP.shape)) {
			throw new Error("Incompatible shapes");
		}
		if (!arrayA.dataType.equals(arrayOutP.dataType)) {
			throw new Error("Incompatible data type");
		}
	} else {
		arrayOutP = {
	...
	jsmath.lupdecomposition(arrayOutL.data, arrayOutU.data, arrayOutP.data, arrayA.shape[0]);
...
function onMessage(event) {
	...
	switch (request.type) {
		...
		case Request.Type.DOT_OPERATION:
			...
			break;
		case Request.Type.LUP_DECOMPOSITION:
			var LUPDecompositionRequest = request.LUPDecompositionRequest;
			LUPDecomposition(request.id,
				LUPDecompositionRequest.idA,
				LUPDecompositionRequest.idOutL,
				LUPDecompositionRequest.idOutU,
				LUPDecompositionRequest.idOutP);
			break;
